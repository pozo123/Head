...
function loadCheckboxesPermisos(){
    firebase.database().ref(rama_bd_inges).orderByChild("nombre").equalTo($('#' + id_colaborardor_ddl_permisos + " option:selected").val()).once("child_added").then(function(snapshot){
        var perms = snapshot.val().permisos;
        if(perms.alta_colaborador === true)
            $('#' + id_alta_colaborador_checkbox_permisos).prop('checked', true);
        else
            $('#' + id_alta_colaborador_checkbox_permisos).prop('checked', false);

            if(perms.alta_obra === true)
            $('#' + id_alta_obra_checkbox_permisos).prop('checked', true);
        else
            $('#' + id_alta_obra_checkbox_permisos).prop('checked', false);

        if(perms.alta_cliente === true)
            $('#' + id_alta_cliente_checkbox_permisos).prop('checked', true);
        else
            $('#' + id_alta_cliente_checkbox_permisos).prop('checked', false);

        if(perms.reporte === true)
            $('#' + id_reporte_checkbox_permisos).prop('checked', true);
        else
            $('#' + id_reporte_checkbox_permisos).prop('checked', false);

        if(perms.perfil === true)
            $('#' + id_perfil_checkbox_permisos).prop('checked', true);
        else
            $('#' + id_perfil_checkbox_permisos).prop('checked', false);
    });
}

$('#' + id_guardar_button_permisos).click(function () {
    var perm = {
        alta_colaborador: $('#' + id_alta_colaborador_checkbox_permisos).checked,
        alta_obra: $('#' + id_alta_obra_checkbox_permisos).checked,
        alta_cliente: $('#' + id_alta_cliente_checkbox_permisos).checked,
        reporte: $('#' + id_reporte_checkbox_permisos).checked,
        perfil: $('#' + id_perfil_checkbox_permisos).checked,
    }
    firebase.database().ref(rama_bd_inges + "/" + $('#' + id_colaborardor_ddl_permisos + " option:selected").val() + "/permisos").update(perm);
    //Hecho con clase:
