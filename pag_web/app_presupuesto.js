...
function loadAtn(){
    ...
    firebase.database().ref(rama_bd_obras + "/" + $('#' + id_obra_ddl_presupuesto + " option:selected").val()).once('value').then(function(snapshot){
        ...
        firebase.database().ref(rama_bd_clientes).orderByChild('nombre').equalTo(obra_selec.cliente).on('child_added').then(function(snapshot){
            var cliente = snapshot.val();
            firebase.database().ref(rama_bd_clientes + "/" + cliente.nombre + "/atencion").orderByChild('nombre').once("child_added").then(function(snapshot){
            ...
            firebase.database().ref(rama_bd_clientes).orderByChild("nombre").equalTo(obra_selec.cliente).once('child_added').then(function(snapshot){
                ...
                firebase.database().ref(rama_bd_obras + "/" + obra_selec.nombre + "/presupuestos/" + $('#' + id_nombre_presupuesto).val()).once('value').then(function(snapshot){
                    ...
                    var pdfPresupuesto = {
                        ...
                        footer: function(currentPage) { return { fontSize:8,alignment: 'center', text: (currentPage == 2) ? 'Av. Constituyentes 561 Int. 101a, Col. América, Miguel Hidalgo, Ciudad de México, C.P. 11820, Tel. 6273 7900 , email: proyectos@headingenieria.mx' : '' }; },
