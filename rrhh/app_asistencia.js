...
function cargaEntradasAsistencia(year,semana){
    firebase.database().ref(rama_bd_obras_magico).orderByChild("nombre").equalTo($('#' + id_obra_ddl_asistencia + " option:selected").val()).once('child_added').then(function(snapshot){
        var procesos = [];
        terminados = [];
        var count_proc = 0;
